"""Tests for the agent supply-chain integrity gate."""
...
import check_agent_supply_chain_integrity as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "agent-supply-chain-integrity-transcript.json"
        ...
        for edge in edges:
            ...
            assert "package_class" in edge
            assert "package_id" in edge
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_unauthorized_provenance_or_unsigned_package_edges"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "agent-supply-chain-integrity.json"
