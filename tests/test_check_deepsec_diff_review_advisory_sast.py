"""Tests for the DeepSec diff-review advisory SAST gate."""
...
import check_deepsec_diff_review_advisory_sast as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "deepsec-diff-review-advisory-transcript.json"
        ...
        for edge in edges:
            ...
            assert "untrusted_input_source" in edge
            assert "dangerous_sink" in edge
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_untrusted_to_dangerous_sink_edges"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "deepsec-diff-review-advisory-sast.json"
