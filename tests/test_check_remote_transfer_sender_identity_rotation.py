...
import check_remote_transfer_sender_identity_rotation as gate
...
def test_summarize_passes_when_all_sender_identity_rotation_invariants_hold(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)

    assert payload["schema"] == "ao-operator/remote-transfer-sender-identity-rotation/v1"
...
def test_clean_case_records_no_observed_errors(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)
    by_id = {case["id"]: case for case in payload["cases"]}
    assert by_id["clean_post_rotation_bundle_accepted"]["observed_errors"] == []
...
def test_cli_writes_status_json_with_pass_verdict(tmp_path, capsys):
    output = tmp_path / "run-artifacts/sender-identity-rotation.json"
    ...
    assert saved["schema"] == "ao-operator/remote-transfer-sender-identity-rotation/v1"
