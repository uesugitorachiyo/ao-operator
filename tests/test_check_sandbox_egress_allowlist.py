"""Tests for the sandbox egress allowlist gate."""
...
import check_sandbox_egress_allowlist as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "sandbox-egress-allowlist-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        egresses = data["egresses"]
        assert isinstance(egresses, list) and egresses, case_id
        for egress in egresses:
            assert egress["op"] == "register"
            assert "egress_class" in egress
            assert "egress_id" in egress
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_unallowlisted_or_bypassed_egress_attempts"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "sandbox-egress-allowlist.json"
