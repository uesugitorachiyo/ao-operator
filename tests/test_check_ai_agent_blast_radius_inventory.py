...
import check_ai_agent_blast_radius_inventory as gate
...
def test_summarize_passes_when_all_blast_radius_invariants_hold(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)

    assert payload["schema"] == "ao-operator/ai-agent-blast-radius-inventory/v1"
    ...
    assert "command" in payload["categories"]
    assert "destructive_action" in payload["categories"]
    assert "credential" in payload["categories"]
    assert "provider_dispatch" in payload["categories"]
    assert "release_artifact" in payload["categories"]
    assert "high" in payload["blast_radius_levels"]


def test_clean_case_records_no_observed_errors(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)
    by_id = {case["id"]: case for case in payload["cases"]}
    assert by_id["clean_inventory_classified_and_gated"]["observed_errors"] == []
...
def test_cli_writes_status_json_with_pass_verdict(tmp_path, capsys):
    output = tmp_path / "run-artifacts/ai-agent-blast-radius-inventory.json"
    ...
    assert saved["schema"] == "ao-operator/ai-agent-blast-radius-inventory/v1"
