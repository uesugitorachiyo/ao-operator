"""Tests for the agent credential scope narrowing gate."""
...
import check_agent_credential_scope_narrowing as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "agent-credential-scope-narrowing-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        credentials = data["credentials"]
        assert isinstance(credentials, list) and credentials, case_id
        for credential in credentials:
            assert credential["op"] == "register"
            assert "credential_class" in credential
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_credential_scope_widening"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "agent-credential-scope-narrowing.json"
