"""Tests for the MCP / Tool poisoning detection gate."""
...
import check_mcp_tool_poisoning_detection as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "mcp-tool-poisoning-transcript.json"
        ...
        for edge in edges:
            ...
            assert "descriptor_classification" in edge
            assert "tool_name" in edge
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_mcp_or_tool_poisoning_indicators"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "mcp-tool-poisoning-detection.json"
