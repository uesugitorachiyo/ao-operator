"""Tests for the agent log redaction round-trip gate."""
...
import check_agent_log_redaction_round_trip as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "agent-log-redaction-round-trip-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        entries = data["entries"]
        assert isinstance(entries, list) and entries, case_id
        for entry in entries:
            assert entry["op"] == "register"
            assert "log_class" in entry
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_round_trip_recoverable_secret_or_personal_path_in_redacted_output"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "agent-log-redaction-round-trip.json"
