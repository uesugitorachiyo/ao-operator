"""Tests for the prompt-injection escape boundary gate."""
...
import check_prompt_injection_escape_boundary as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "prompt-injection-escape-boundary-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        sections = data["sections"]
        assert isinstance(sections, list) and sections, case_id
        for section in sections:
            assert section["op"] == "register"
            assert "section_class" in section
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_role_spoofing_or_attacker_controlled_system_prompt_appended"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "prompt-injection-escape-boundary.json"
