"""Tests for the tool-result cache poisoning defense gate."""
...
import check_tool_result_cache_poisoning_defense as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "tool-result-cache-poisoning-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        cache_entries = data["cache_entries"]
        assert isinstance(cache_entries, list) and cache_entries, case_id
        for entry in cache_entries:
            assert entry["op"] == "register"
            assert "entry_class" in entry
            assert "cache_entry_id" in entry
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_tool_result_cache_poisoning"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "tool-result-cache-poisoning-defense.json"
