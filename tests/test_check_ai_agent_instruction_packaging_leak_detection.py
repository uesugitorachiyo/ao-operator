"""Tests for the AI agent instruction & release packaging leak detection gate."""
...
import check_ai_agent_instruction_packaging_leak_detection as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "instruction-packaging-leak-transcript.json"
        ...
        for edge in edges:
            ...
            assert "source_classification" in edge
            assert "public_artifact_class" in edge
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_instruction_or_packaging_leaks_in_public_artifacts"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "ai-agent-instruction-packaging-leak-detection.json"
