...
import check_remote_transfer_wire_encryption_required as gate
...
def test_summarize_passes_when_all_wire_encryption_invariants_hold(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)

    assert payload["schema"] == "ao-operator/remote-transfer-wire-encryption-required/v1"
...
def test_clean_case_records_no_observed_errors(tmp_path):
    payload = gate.summarize(work_dir=tmp_path)
    by_id = {case["id"]: case for case in payload["cases"]}
    assert by_id["clean_encrypted_bundle_accepted"]["observed_errors"] == []
...
def test_cli_writes_status_json_with_pass_verdict(tmp_path, capsys):
    output = tmp_path / "run-artifacts/wire-encryption-required.json"
    ...
    assert saved["schema"] == "ao-operator/remote-transfer-wire-encryption-required/v1"
