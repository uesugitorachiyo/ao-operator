"""Tests for the per-tenant blast-radius cap gate."""
...
import check_per_tenant_blast_radius_cap as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "per-tenant-blast-radius-cap-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        actions = data["actions"]
        assert isinstance(actions, list) and actions, case_id
        for action in actions:
            assert action["op"] == "register"
            assert "action_class" in action
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_cross_tenant_or_unallowlisted_or_quota_overflow_action_edges"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "per-tenant-blast-radius-cap.json"
