"""Tests for the agent output canary-leak detection gate."""
...
import check_agent_output_canary_leak_detection as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "agent-output-canary-leak-detection-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        outputs = data["outputs"]
        assert isinstance(outputs, list) and outputs, case_id
        for output in outputs:
            assert output["op"] == "register"
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_canary_or_marked_secret_leak_in_output"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "agent-output-canary-leak-detection.json"
