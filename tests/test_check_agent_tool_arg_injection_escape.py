"""Tests for the agent tool-argument injection escape gate."""
...
import check_agent_tool_arg_injection_escape as gate  # noqa: E402
...
def test_each_case_persists_a_per_case_transcript(tmp_path: Path) -> None:
    ...
    for case_id in gate.CASE_IDS:
        transcript_path = tmp_path / case_id / "agent-tool-arg-injection-escape-transcript.json"
        assert transcript_path.exists(), case_id
        data = json.loads(transcript_path.read_text(encoding="utf-8"))
        tool_calls = data["tool_calls"]
        assert isinstance(tool_calls, list) and tool_calls, case_id
        for call in tool_calls:
            assert call["op"] == "register"
            assert "call_class" in call
...
def test_clean_case_has_no_observed_errors(tmp_path: Path) -> None:
    ...
    clean_case = next(
        ...
        if case["id"] == "clean_no_tool_arg_injection_or_breakout_or_polymorphic_coercion"
    )
...
def test_write_output_persists_payload(tmp_path: Path) -> None:
    ...
    out = tmp_path / "agent-tool-arg-injection-escape.json"
