#!/usr/bin/env python3
"""Dry-run remote approval materialization on Ubuntu using a signed bundle."""
...
RETURN_SCHEMA = "ao-operator/remote-approval-materialization-dry-run-return/v1"
VALIDATION_SCHEMA = "ao-operator/mac-ubuntu-remote-approval-materialization-dry-run-validation/v1"
DEFAULT_OUTPUT = f"{STATUS_ROOT}/mac-ubuntu-remote-approval-materialization-dry-run.json"
DEFAULT_REMOTE_USER = signed_transfer.DEFAULT_REMOTE_USER
DEFAULT_REMOTE_REPO = signed_transfer.DEFAULT_REMOTE_REPO
DEFAULT_REMOTE_BASE = "/tmp/ao-operator-remote-approval-materialization-dry-run"
...
def remote_verify_and_materialize_script() -> str:
...
approval_file = Path(str(approval_target))
if approval_file.is_absolute() or ".." in approval_file.parts:
    errors.append("approval file target must be a safe repo-relative path")
approval_file_present_before = approval_file.is_file()
if approval_file_present_before:
    errors.append("approval file must be absent before remote dry-run materialization")
...
if not errors:
    ...
    if result.returncode != 0:
        errors.append("remote approval materialization dry-run command failed")
    ...
    if materialization.get("verdict") != "PASS":
        errors.append("remote materialization dry-run verdict must be PASS")
    if materialization.get("approval_file_written") is not False:
        errors.append("remote materialization dry-run must not write approval file")
    if materialization.get("approval_valid") is not False:
        errors.append("remote materialization dry-run must not produce valid approval")
...
if approval_file_present_after:
    errors.append("approval file must be absent after remote dry-run materialization")
...
payload = {{
    ...
    "materialization_dry_run_passed": materialization.get("verdict") == "PASS" and materialization.get("approval_file_written") is False,
    "approval_file_written": materialization.get("approval_file_written", False),
    "approval_valid": materialization.get("approval_valid", False),
    "approval_file_present_before": approval_file_present_before,
...
def validate_remote_return(
    ...
    *,
    ...
    for key in ("signed_bundle_verified", "signature_verified", "identity_verified", "materialization_dry_run_passed"):
        if payload.get(key) is not True:
            errors.append(f"{key} must be true")
    for key in ("approval_file_written", "approval_valid", "approval_file_present_after"):
...
def build_report(
    *,
    ...
    root = root.resolve()
    transfer_id = transfer_id or f"remote-approval-materialization-dry-run-{utc_stamp()}"
    ...
    if not errors:
        ...
        with tempfile.TemporaryDirectory(prefix="ao-operator-remote-approval-materialization-") as temp:
            ...
            if not errors:
                verify_result = runner(
                    ...
                    input_text=remote_verify_and_materialize_script(),
                    timeout=timeout,
                )
                try:
                ...
                if verify_result.returncode != 0:
                    errors.append("remote approval materialization dry-run failed")
            ...
            if remote_payload:
                ...
                errors.append("remote materialization return manifest missing")
    ...
    payload = {
        ...
        "materialization_dry_run_passed": remote_payload.get("materialization_dry_run_passed") is True,
        "approval_file_written": remote_payload.get("approval_file_written", False),
        "approval_valid": remote_payload.get("approval_valid", False),
        "approval_file_present_after": remote_payload.get("approval_file_present_after", False),
        ...
        "errors": errors,
        "next_safe_command": (
            "Remote approval materialization dry-run passes; continue with remote approval revocation rollback lane."
            if not errors
            else "Fix remote approval materialization dry-run before any remote approval write path."
        ),
    }
...
def validate_committed_report(*, root: Path = ROOT, report_path: str | Path = DEFAULT_OUTPUT) -> dict[str, Any]:
    ...
    for key in ("remote_git_synced", "signed_bundle_verified", "signature_verified", "identity_verified", "materialization_dry_run_passed", "remote_cleanup_absent"):
        if report.get(key) is not True:
            errors.append(f"{key} must be true")
    for key in ("approval_file_written", "approval_valid", "approval_file_present_after", "dispatch_authorized", "live_providers_run", "provider_dispatch"):
    ...
    return {
        ...
        "live_providers_run": False,
        "next_safe_command": (
            "Committed remote approval materialization dry-run report passes."
            if not errors
            else "Refresh remote approval materialization dry-run evidence before continuing."
        ),
    }


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Check Mac-to-Ubuntu remote approval materialization dry-run")
    ...
    parser.add_argument("--remote-base", default=os.environ.get("FACTORY_V3_REMOTE_APPROVAL_MATERIALIZATION_BASE", DEFAULT_REMOTE_BASE))
